...
const normalizePasskey = (passkey) => String(passkey);
...
export const verifyPasskey = async (passkey, storedHash) => {
    const pass = normalizePasskey(passkey);
...
export const encryptData = async (plaintext, passkey, salt) => {
...
export const decryptData = async (encryptedBlob, passkey, salt) => {
    if (!encryptedBlob) {
