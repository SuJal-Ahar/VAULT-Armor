...
app.post("/api/v1/vault", authMiddleware, async (req, res) => {
    try {
        ...
        console.error("❌ Error creating vault:", error?.message || error);
        if (error?.stack)
...
app.post("/api/v1/vault/open", authMiddleware, async (req, res) => {
    try {
        ...
        for (let i = 0; i < vault.items.length; i++) {
            ...
            item.encryptedBlob, passkey, vault.keySalt);
            items.push({
                // @ts-ignore
                id: item._id,
                // @ts-ignore
...
// DELETE ITEM (password) FROM VAULT
app.delete("/api/v1/vault/:vaultId/item/:itemId", authMiddleware, async (req, res) => {
    try {
...
// ADD ITEM
app.post("/api/v1/vault/add-item", authMiddleware, async (req, res) => {
    try {
        const { vaultId, passkey, title, password, link, description } = req.body;
        if (!vaultId || !passkey || !title || !password)
